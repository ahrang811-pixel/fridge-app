        ...
        <div className="flex flex-col gap-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <Section title="3. 외부 서비스로의 정보 전송 (제3자 제공·처리위탁)">
            ...
            <ul className="list-disc pl-5">
              ...
              <li>
                <strong>인스타그램(instagram.com)</strong> (&lsquo;인스타그램으로
                추가&rsquo; 기능): 입력한 게시물 링크로 서비스 서버가 공개
                게시물 페이지에 접속해 캡션(본문) 텍스트를 가져오려고
                시도합니다. 자동으로 가져오지 못하면 이용자가 캡션을 직접
                붙여넣을 수 있습니다.
              </li>
              <li>
                <strong>Google Gemini API</strong>: (1) &lsquo;냉장고 재료로
                레시피 추천&rsquo; 기능 사용 시 현재 등록된 식재료 이름
                목록이, (2) &lsquo;유튜브 링크로 추가&rsquo;·&lsquo;인스타그램으로
                추가&rsquo; 기능 사용 시 영상 제목·설명란·댓글 또는 인스타그램
                캡션 텍스트가 레시피 정보 분석을 위해, (3) 재고 관리 화면의
                식재료 상세정보 표시를 위해 식재료 이름이 보관정보(보관 방법별
                안내, 최대 보관기한, 구입 꿀팁) 생성 목적으로 Google Gemini
                API로 전송됩니다.
              </li>
              <li>
                <strong>Pollinations.ai</strong>: 재고 관리 화면의 식재료
                카드에 쓰이는 일러스트 이미지를 생성하기 위해 식재료 이름이
                전송됩니다(API 키 없이 이용 가능한 공개 이미지 생성 서비스).
              </li>
              <li>
                <strong>Supabase</strong>: 계정 정보와 이용자가 입력한
                냉장고·레시피·식단 데이터는 인프라 제공업체인 Supabase의
                서버에 저장됩니다 (개인정보처리위탁). 또한 위에서 생성된
